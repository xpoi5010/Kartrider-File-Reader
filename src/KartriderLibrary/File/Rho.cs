...
namespace KartRider.File
{
    public class Rho : IDisposable
    {
        ...
        private Dictionary<uint, RhoBlockInfo> Blocks;
        ...
        public Rho(string FileName)
        {
            ...
            using (MemoryStream ms = new MemoryStream(part2Data))
            {
                ...
                Blocks = new Dictionary<uint, RhoBlockInfo>(BlockCount);
            }
            ...
            for (int i = 0; i < BlockCount; i++)
            {
                if (Version == 1.0d)
                {
                    var blockInfo = reader.ReadBlockInfoOld(BlockInfoKeyOld);
        ...
        internal RhoBlockInfo GetBlockInfo(uint Index)
        {
